#!/usr/bin/env python3
"""Detect a ball, map 2D camera point to fake TCP XY, and call movep_to_pose.py."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--calibration", type=Path, default=Path("fake_tcp_planar_calibration.json"))
    ...
    parser.add_argument("--rpy", type=parse_rpy, default=(176.0, 45.0, -170.0))
...
def load_calibration(path: Path) -> tuple[dict[str, object], np.ndarray]:
    ...
    return payload, np.asarray(payload["affine_2x3"], dtype=float)
...
def main() -> int:
    ...
    target_xy_m = affine @ source
    rx, ry, rz = args.rpy
    target = f"{target_xy_m[0] * 1000.0:.3f},{target_xy_m[1] * 1000.0:.3f},{args.fixed_z_mm:.3f},{rx:.3f},{ry:.3f},{rz:.3f}"
    ...
    print(f"pixel={pixel}")
    print(f"fake_tcp_xy_m=({target_xy_m[0]:.6f}, {target_xy_m[1]:.6f})")
